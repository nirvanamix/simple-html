jQuery(document).ready(function($) {
	...
	const auncor = {
		fun: function(){
			jQuery('a.auncor').click(function(event){
				...
				if(scroll_t<=60){
					$('html, body').stop(true, false).animate({scrollTop: $(target).offset().top - height_header}, 900);
				}
				else{
					$('html, body').stop(true, false).animate({scrollTop: $(target).offset().top - height_header}, 900);
				}
	...
	const area_fixed_button = {
		fun: () => {
			$(window).scroll(function(event) {
				const scroll_top = parseInt($(this).scrollTop());
				const formula = $('.start-show-link').offset().top + $('.start-show-link').outerHeight() - $('header').outerHeight();
				const formula_stop = $('footer').offset().top - $(window).outerHeight();
